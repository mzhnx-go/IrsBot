# Checklist - IrsBot Agent Platform Implementation (v2)
...
- [ ] **11.1 端到端集成测试**
  ...
  - [ ] `tests/integration/` 测试通过

- [ ] **11.2 性能与安全**
  - [ ] P99 < 10s + API Key AES-256 加密 + Rate Limiting + 输入限制
  - [ ] Milvus 降级方案 (ChromaDB) 可切换

---

## Success Criteria

- [ ] **SC1**: 端到端对话 (LangGraph ReAct 循环完整)
- [ ] **SC2**: 至少 2 种 LLM (LangChain ChatModel 切换)
- [ ] **SC3**: 至少 3 种内置工具 (LangChain @tool)
- [ ] **SC4**: MCP Server 连接 + LangChain ToolBridge 调用
- [ ] **SC5**: LangChain TextSplitter + Milvus + 检索返回
- [ ] **SC6**: Skill 安装 + Agent 读取 SKILL.md
- [ ] **SC7**: WebUI 聊天 + 管理页面
- [ ] **SC8**: Pipeline 执行所有 Stage
- [ ] **SC9**: Alembic 迁移同时覆盖 SQLModel 和 SQLAlchemy 2.0 模型
