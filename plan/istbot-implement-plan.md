...
| **D2.1** | **注册入口默认关闭** | `USERS_OPEN_REGISTRATION=false` → `POST /users/signup` 直接 403，账号只由管理员创建（`41cccfd`）。沿用 D6 的「高危能力默认关」范式 |
...
| 注册 | **关闭**（`USERS_OPEN_REGISTRATION=false`，已实现：`POST /users/signup` 直接 403，账号只由管理员创建） | 开放 |
| 邮件 | 关闭 | 开启 |
...
| **D10** | **无自助注册入口**：匿名调用 `POST /users/signup` 必须被拒（403），且响应不泄露账号是否存在 | ✅ **达成**（2026-09-18 实机验证）：`USERS_OPEN_REGISTRATION=false` → 403 + 固定话术；已存在 / 不存在邮箱的**响应逐字节一致**（防枚举）；闸门置于查重之前 |
...
- 开关：`USERS_OPEN_REGISTRATION`（默认 **False**，沿用 D6 的 `ENABLE_SHELL` / `ENABLE_FILE_WRITE` 范式）
