...
| **D2** | **本地版是单用户** | **Phase 11（Provider 多用户化）降级** —— 串 Key 在单用户下不存在；仅保留 `_chat_cache` 淘汰与 `is_default` 约束两项 |
...
| 多租户要不要做？ | **不做** ➖。**已随网页版拆分出去** | 本地单用户 |
| Provider 归属？ | 部署者自己的 Key，**网页端填写** | 单用户；无需多租户隔离 |
...
- `Provider` 读取函数保留可选的归属参数位（**留接口不留逻辑**）
...
| 「不做多租户，降级为数据归属校验」（IM 轮） | **不做多租户** ✅（结论相同，理由不同） | 本地单用户 + 网页版已拆分 |
| 「多租户恢复，且是 BYOK」（多用户轮） | **已推翻** | D2：本地单用户 |
| 「Provider 多用户隔离是 P0」（多用户轮） | **降级** | D2：单用户下不存在串 Key |
...
| Phase 1 | 项目基础架构搭建 | ✅ 完成 |
| Phase 2 | 对话管理系统 | ✅ 完成 |
| Phase 3 | Provider 系统 (LangChain 适配层) | 🔶 Chat + Embedding 完成；**缺缓存淘汰**（单用户下同样存在，见 8.4） |
...
| **Phase D1** | **Compose 本地化改造**（剥离 Traefik/域名/adminer/HTTPS；补全 Milvus 全家桶；前端改后端托管） | ⬜ 未开始 ｜ **P0 ★ 施工第一步**（M1/M2/M3 调研须先做） |
| **Phase D2** | **一键启动脚本**（Windows `.cmd` / macOS·Linux `.sh` 双份） | ⬜ 未开始 ｜ **P0 ★ 依赖 D1** |
| **Phase D3** | **首次启动引导**（SECRET_KEY 落盘 + 随机管理员密码 + 打印地址） | ⬜ 未开始 ｜ **P0 ★ 可与 D2 合并实现** |
| **Phase D4** | **端到端验证**（干净机器跑通全功能；测试矩阵；README） | ⬜ 未开始 ｜ **P0 ★ 依赖 D1+D2+D3** |
| **Phase D5** | **数据迁移**（本机开发数据 → 容器；**最高风险，强制备份**） | ⬜ 未开始 ｜ **P0 ★ 必须严格在 D4 之后** |
| **Phase D6** | **工具权限开关**（`shell` / `file_write` 默认关闭 + 路径白名单） | ⬜ 未开始 ｜ **P0**（独立可插，不阻塞 D1–D4） |
| **Phase D7** | **交付**（交付说明 / 备份恢复文档 / 卸载说明） | ⬜ 未开始 ｜ P0 ｜ 依赖 D4+D5 |
...
- [ ] **3.5 缓存淘汰缺失（本轮新识别，P0）**
  ...
  - **与用户数无关**：单用户下同样存在（原 Phase 11 的「多用户隔离」部分已随 D2 取消，仅此项保留）
  - 计划：加 LRU 或 TTL 上限
  - 相关：`get_chat_model()` / `get_embedding_model()` 签名**保留可选归属参数位**（`user_id: UUID | None = None`），单用户下忽略 —— **留接口不留逻辑**，供另一个项目复用
...
| **D1** | **一条命令起全栈**：已装 Docker 的机器上 `docker compose up -d` 后 6 个服务（db/etcd/minio/milvus/backend/frontend）全部 healthy | ⬜ 见 D1 |
| **D2** | **`http://localhost:8000` 可直接对话**：浏览器打开即用，**无需手动建库、无需改任何配置文件** | ⬜ 见 D1+D2 |
| **D3** | **前端单端口**：前端由后端托管静态产物，**不再需要 5173 dev server** | ⬜ 见 D1.3 |
| **D4** | **首次启动零配置**：自动生成 SECRET_KEY **并落盘**、自动建表、自动建管理员、终端**打印随机密码与访问地址** | ⬜ 见 D3 |
| **D5** | **重启不丢登录态**：`docker compose restart` 后已登录会话仍然有效（验证 SECRET_KEY 未变） | ⬜ 见 D3 |
| **D6** | **`shell` / `file_write` 工具默认关闭**，设置页可显式开启 | ⬜ 见 D6 |
| **D7** | **开发数据完整迁移**：本机 126 会话 / 108 消息 / 22 知识库 / 21 Persona / 17 MCP 全部可用 | ⬜ 见 D5 |
| **D8** | **可分发**：把仓库拷给他人，对方装好 Docker 后同样一条命令跑通（验证无本机隐式依赖） | ⬜ 见 D4.3 |
| **D9** | **测试基线不退化**：294 单测 + 18 集成测试在容器环境下仍全过 | ⬜ 见 D4.2 |
| ~~SC11~~ | ~~多用户 BYOK：A 用户绝不会用到 B 用户的 Key~~ | ➖ **取消**（D2：本地单用户） |
...
> **D1–D9 是本项目「本地可用」的验收标准**，优先级等同 P0。详见 [local-deployment-plan.md](./local-deployment-plan.md)。
...
|---|---|---|---|
| **Provider 体系** | 5 类能力 + 装饰器注册 + 动态 import | 🔶 Chat + Embedding，**读取函数缺归属参数位**（留接口即可） | P0（cache 淘汰）｜P1（Rerank） |
...
> **已移出本表的事项**（原编号）：多用户隔离相关的 3 项（原 #1/#2/#3）随 D2 取消；在线部署开关分层（原 #14）与多用户配额（原 #24）随 D1 移交另一个项目。
...
> - **Phase 11 Provider 多用户化** → **降级**（单用户下串 Key 不存在），仅保留 2 项，见 8.4
...
| ~~Phase 11~~ | ~~Provider 多用户化（BYOK）~~ | ➖ | **降级**，仅保留 2 项 | — | 8.4 |
...
6. **原 Phase 11 / 13 / 16 均已移出**——11 因单用户降级，13 因 D4 作废，16 因网页版拆分。它们的价值转移到了别处，不是"延期"，是"不在这里做"。
...
| ~~Phase 11~~（降级） | 本文档 **8.4** | 为什么不再适用 + 保留的 2 项 + 「留接口不留逻辑」 |
...
### 8.4 原 Phase 11 的处理（降级说明）

> **Phase 11「Provider 多用户化（BYOK）」已因 D2（本地单用户）降级。**
> 串 Key 问题在单用户下**不存在** —— 部署者只有自己一个账号。

#### 为什么原 Phase 11 的全部内容不再适用

| 原任务 | 现在的处置 |
|---|---|
| 11.1 读取路径贯穿 `user_id`（必填参数） | ➖ 不做。单用户下恒为同一值，加参数只是增加噪音 |
| 11.2 缓存键隔离（键含 `user_id`） | 🔶 **部分保留** → 只保留「缓存淘汰」部分，见下 |
| 11.3 传参链路改造（WS / REST / Agent 节点） | ➖ 不做 |
| 11.4 多用户隔离回归测试 | ➖ 不做 |
| 11.5 `is_default` 唯一约束 | 🔶 **保留**，但理由变了（见下） |
...
> 另一个项目（网页版，多用户）会复用本代码库。因此：

- Provider 读取函数**保留可选的归属参数位**（`user_id: UUID | None = None`），单用户下忽略它
- **不在本项目内实现隔离逻辑**，只留签名 —— 对方接入时补逻辑即可
- 这样对方**不必推倒重来**，本项目也**不必背负用不上的复杂度**
