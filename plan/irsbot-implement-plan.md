...
| **Phase 15** | 前端完善（对话体验 + 配置台管理页） | ✅ **已完成**（2026-09-20，15.1 / 15.2 / 15.3 全部闭环，e2e 60 passed，详见 PROGRESS 待办 11）｜ P1 |
...
### 8.5 Phase 15: 前端完善（对话 + 配置台）（P1）—— ✅ 已完成（2026-09-20）

**目标**：让用户**完全通过网页**完成「填 Key → 对话 → 管 KB / MCP / Skill」全流程。✅ 已达成。

#### 15.1 对话体验（产品主体）
- [x] 会话列表管理（新建 / 重命名 / 删除 / 搜索）
- [x] 消息操作（重新生成 / 复制 / 编辑重发 / 删除）
- [x] 流式渲染打磨（打字机、代码块高亮、Markdown 表格）
- [x] 工具调用可视化（折叠面板展示入参/结果，当前已有基础）
- [x] 引用与检索来源展示（RAG 命中片段 + 相似度）
- [x] 中断当前生成（调用已有 `interrupt`）
- [x] 错误态与重试提示中文化
- [x] 移动端适配

#### 15.2 配置台管理页（关键）
- [x] **配置闭环**：首次进入 → 引导填 Key → 首轮对话（空状态引导）
- [x] Provider 管理页（10.3 / 10.5 收尾）
- [x] 知识库页：KB 列表 / 文档上传 / 分块与检索参数 / 检索测试
- [x] MCP 页：列表 / 连接测试 / 工具预览
- [x] 技能页：列表 / 上传 / 详情
- [x] Persona 页：列表 / 编辑 / 会话绑定
- [x] 会话管理页：列表 / 查看历史（含工具调用折叠）/ 启用停用 / 删除
- [x] 统计页：token 用量 / 耗时 / 工具调用（依赖 14.3）
- [x] 设置页：账号 / 密码 / **工具权限开关（依赖 D6）** / 外观

#### 15.3 其它
- [x] 侧边栏信息架构重组：对话 / 知识库 / MCP / 技能 / 模型源 / 会话 / 统计 / 设置
- [x] Logo 替换（去掉 FastAPI SVG）
- [x] 每个页面至少 1 个 Playwright e2e（**在容器环境跑**）
...
| 变更 | 内容 |
|---|---|
| **文件重命名** | `progress.md` → `istbot-implement-plan.md`（按用户要求）→ 2026-09-21 更正拼写为 `irsbot-implement-plan.md`（项目名为 IrsBot） |
