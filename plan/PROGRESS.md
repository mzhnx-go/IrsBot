...
**✅ 已闭环（无需再做）**：D1–D7 全部完成 ｜ D5 判定不迁移 ｜ §10.6 Provider a–f 实测 ｜ d 项缺陷修复 ｜ 系统提示词 ｜ 注册闸门（方案 a）｜ **容器同步（2026-09-18）** ｜ **KB 管理 UI 基础页（2026-09-18，列表/新建/删除 + 侧边栏入口）** ｜ **KB 详情页 + 删除文档端点 + docx 解析（2026-09-19，方案 A 一至三章）**。
...
| 6 | ~~补后端「删除文档」端点~~ **已完成（2026-09-19）**：`DELETE /api/v1/kb/{kb_id}/documents/{doc_id}` 已实现（三处清理：Milvus 按 doc_id / 磁盘 / DB，4 条回归测试）；前端详情页就绪，**剩「重跑 generate-client 接通 deleteDoc」在方案 A 第四章** | KB 管理 UI 施工中发现（2026-09-18） | 后端 ✅；前端接线待做 |
