...
**✅ 已闭环（无需再做）**：D1–D7 全部完成 ｜ D5 判定不迁移 ｜ §10.6 Provider a–f 实测 ｜ d 项缺陷修复 ｜ 系统提示词 ｜ 注册闸门（方案 a）｜ **容器同步（2026-09-18）** ｜ **KB 管理 UI 基础页（2026-09-18，列表/新建/删除 + 侧边栏入口）**。
...
| 6 | **补后端「删除文档」端点**：`DELETE /api/v1/kb/{kb_id}/documents/{doc_id}`（当前只有上传/列表，前端 `useKnowledgeBase.ts` 的 `deleteDoc` 是抛错占位） | KB 管理 UI 施工中发现（2026-09-18） | 待做；实现时同步重跑 `generate-client` 并接通前端 `deleteDoc` |
