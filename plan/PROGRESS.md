...
| **测试基线** | ✅ **全量最新实跑：`pytest ../tests -q` → 321 passed, 0 failed（123.29s，2026-09-18）**（上次 311；新增 = §10.6 修复 7 条 + 注册闸门 3 条；工作区另有未提交的 Provider 多租户加固测试，一并跑通）；后端 `py_compile` ✅；⚠️ **前端 `tsc` 本次未跑** —— `frontend/node_modules` 依赖未安装（目录内仅 vite 缓存），无法执行 |
