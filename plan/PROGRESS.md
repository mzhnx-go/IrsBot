...
> 全局规划与架构决策见 [irsbot-implement-plan.md](./irsbot-implement-plan.md)（少变）
...
| **文档状态** | ✅ **已一致**（含 2026-09-18 **D2 语义拆分回查**）：`irsbot-implement-plan.md` **第六轮同步**——把「Phase 11 因单用户整体降级」拆为 **(A) 多用户运营化**（降级不做）与 **(B) 归属隔离**（已实现为硬约束），共修 **15 处活引用**（0.0 决策表 / 0.2 / 0.4 / 0.5 / 一节阶段表 / Phase 3 / Phase 3.5 / SC11 / SC D10 / 6.1 能力矩阵 / 8.0 / 8.1 / 8.4 / 5.x 已移出表 / 第九节变更说明），`~~划掉~~` 与历轮留痕按「追溯性引用」**一律保留**；`local-deployment-plan.md` 已升 **v7.2**；详见第八节末「文档同步：D2 语义拆分」 |
...
**漂移是什么**：`irsbot-implement-plan.md` 写「因 D2（本地单用户），Phase 11 **整体降级** —— 串 Key 在单用户下不存在」，并把它落成一条**义务**：「Provider 读取函数保留可选归属参数位（`user_id: UUID | None = None`），单用户下忽略 —— **留接口不留逻辑**」。
...
| # | 待办 | 依据 | 状态 |
|---|---|---|---|
| 16 | ⬜ **P9 三类能力集成（P5 地基已就绪，未做）**：① **P9a 嵌入 / 重排序走 Provider 体系**——`vec_store.get_embedding_model()`（`core/knowledge_base/vec_store.py:17-23`）与 `reranker.rerank()`（`core/knowledge_base/reranker.py`）现在**直接读 `settings.EMBEDDING_*`**，要改成「先查 `capability=embedding` / `rerank` 的默认源，查不到回落 `.env`」——这正是 P5 详案承诺的「读时兼容 + `.env` 两个值都在时启动横幅提示可迁移」；⚠️ **难点：`user_id` 目前根本没到那一层**，须从 `builtins/kb_query.py:71` 与 KB 查询路由一路透传到 `VectorStore` / `HybridRetriever` / `rerank()`（`ProviderManager.get_embedding_model` 是**死代码**，活链路不走它）；② **P9b STT**：聊天输入框语音按钮 → `MediaRecorder` 录音 → `POST /agent/audio/transcriptions`（后端转 openai 兼容 `audio/transcriptions` 上游）；③ **P9c TTS**：回复气泡「朗读」→ `POST /agent/audio/speech` → 前端 `Audio` 播放（**先整段合成，不做流式**）。 | [providers-ui-plan.md](./providers-ui-plan.md) §六 P9 / [handoff-providers-p5-p9.md](./handoff-providers-p5-p9.md) | 🔶 **P9a / P9b / P9c 均已落地**（`ecde963` 嵌入重排序 / `8ab93d8`+`b9aba84` 语音）；P9 全部完成 |
| 15 | ⬜ **聊天路径的 Key 熔断未接（P8 遗留缺口，待决）**：多 Key 轮换只接了余额查询与模型列表两处 `mark_key_failure`（`api/routes/providers.py:173/300/303/311`），**Agent 聊天流的失败分支没接**（`agent_ws.py:504` 的 `except Exception` 只记日志 + 回中文提示）→ 「某把 Key 已失效」在聊天路径上**只轮换不熔断**：下次仍会按 `last_used_at` 最旧把它挑出来。补法与难点见 2026-09-21 进度记录的「P8 缺口」段落。 | P8 交接文档 §二 | ⬜ 待决 |
...
| 2026-09-16 | 文档体系重排：`progress.md` 并入 `irsbot-implement-plan.md`；新建本文档承担运行态 |
...
| 2026-09-16 | **文档一致性修复（原地改写）**：`irsbot-implement-plan.md` 第六至十节全部对齐新定位；第八节由「Phase 11–16 施工手册」降为「战略地图」+ 新增 8.4 降级说明 / 8.6 迁移清单；`astrbot-architecture.md` 定位行与第七/八节优先级同步（原本还写着「IM 机器人框架」） |
...
| 2026-09-16 晚 | **`irsbot-implement-plan.md` 全量重写完成**：第零节（0.1–0.5）、第一节（+新增 1.1 测试基线）、第四节 SC（L1–L8 → **D1–D9**）、第五节（**删除原 P0 #1**，新增 5 项）、第六/七节、第八节全部（8.0 总览 → **D1–D7**、依赖图、优先级矩阵、里程碑 M1–M7）、第九/十节 |
...
| 2026-09-21 | **✅ 获取模型列表 / 自定义模型落地（P7，两笔提交）**。用户反馈「无法获取模型列表」——该按钮当时是 P7 占位（禁用待实现），本轮实现。**后端**：新表 `provider_models`（(provider_id, model_id) 唯一、级联删除）+ 迁移 `c5d9e2a4b8f1`；新模块 `provider_models.py` 适配三种上游协议（openai 兼容 Bearer / anthropic x-api-key+版本头 / gemini key 参数 + models/ 前缀剥离），高级配置全生效；路由四端点（GET 列表 / POST fetch / POST 自定义 / DELETE），fetch 用供应商自身 Key 调上游并 upsert 幂等，失败映射中文 502/504（Key 无效/不可达/超时）。**前端**：`ModelsSection`——进入详情即加载清单、获取按钮显式触发（成功 toast 计数、失败透传中文明细）、自定义模型行内添加、hover 删除、搜索本地过滤。**测试**：`test_provider_models.py` 10 条（三协议构造与解析、upsert 幂等、重复 409、越权 404、上游 401 映射、排序）。**踩坑**：① 测试 monkeypatch httpx.AsyncClient 时嵌套捕获导致递归与「第一个 handler 永远生效」——原类必须模块级捕获一次；② 迁移中 `index=True` 与显式 create_index 同名冲突（DuplicateTable），二选一。**实机验证**：真拉 Agnes 上游 **12 个模型**落库渲染、自定义添加/删除闭环、e2e 24 passed |
...
| 2026-09-21 | **✅ 多 API Key 轮换（P8）实机验收通过 + e2e 补测 + 缺口留档**。**提交链** `9ee9c14`（后端：`provider_keys` 表 + 迁移 `e2f6b3c7a9d4` 含 api_key 回填、轮换/冷却、create/update 密钥行同步）→ `67d2e21`（前端：「添加更多」面板——批量粘贴/打码清单/启停/删除）→ `14d2621`（迁移修复）。**落地语义**：`resolve_api_key(pc)` 在「**启用且未冷却**」的行里按 `last_used_at` 最旧选（`ProviderKey.is_active` 直接进 `where`，所以**启停是真的、不是装饰**），选中即刷 `last_used_at`；连败 `KEY_FAIL_THRESHOLD=3` 次进 `KEY_COOLDOWN_MINUTES=5` 分钟冷却，成功后 `mark_key_success` 只清计数不清冷却；全部冷却时抛中文 `RuntimeError`（「所有 API Key 都处于失败冷却中…」）；**无行时回落 `api_key` 列**（兼容手工改库/异常态）。**实机验收（2026-09-21）**：新增 `frontend/tests/provider-keys.spec.ts`——在 8000 实机 dist 上按行定位 Agnes（`data-provider-name` / base_url 文本，不靠顺序）→ 展开面板 → 批量粘贴 2 把标记 Key → 断言**只出掩码**（`sk-e****1111`，且面板内不含明文、输入框已清空）→ 停用一把 → **reload 后仍未勾选**（证明落库而非乐观更新）→ 重新启用 → 删两把 → **行数回到基线**；**自清理 + 幂等**（进场先删上次失败运行的残留），全程不碰 Agnes 上已有的真实 Key（断言是「基线 ±2」而非绝对行数）。实跑 **2 passed / 8.7s**（`--workers=1 --timeout=120000`，当时空闲内存 1.84GB）。旁证：API 直查 Agnes 密钥行 = 1 行真实 Key（`sk-0****T2ez`，active），用例跑完仍是 1 行；匿名 GET `/providers/{id}/keys` = **401**。后端已有的 `tests/api/routes/test_provider_keys.py` 覆盖轮换轮询 / 全冷却报错 / 回落列 / 连败冷却 / create-sync / PATCH 重置单行 / 路由打码去重 / 启停删除 / 越权 404。**P8 缺口（未补，待决）**：聊天路径失败不走熔断。⚠️ **交接文档给的补法类型写错了**：它建议判 `httpx.HTTPStatusError`——那只适用于余额/模型拉取（那两处直连 httpx）；**聊天走 LangChain → `openai` SDK**，抛的是 `openai.AuthenticationError`/`RateLimitError`（实测容器内 `openai 1.109.1`：这三者均为 `APIStatusError` 子类）与 `openai.APIConnectionError`/`APITimeoutError`，**永远不可能是 `httpx.HTTPStatusError`**（用 anthropic/gemini 源时又是另一套 SDK 异常）。**建议补法**：① 鸭子类型判异常——取 `getattr(exc, "status_code", None)` 或 `exc.response.status_code`，只对 **401/403/429** 与连接/超时类调 `mark_key_failure`（**400 绝不熔断**：那是请求本身的问题，熔 Key 会误伤）；② 把选中的行 id 传出来——`resolve_api_key` 已返回 `(plain, key_row_id)`，但 `get_chat_model` 只在缓存键里用了它（`provider.py:315-319`）没往外给，`agent.py:123` 也丢弃了 → 最小改动是在 `ProviderManager` 上留一个 `last_resolved_key_id`，`Agent.__init__` 记下供 `agent_ws` 失败时调用（改 `get_chat_model` 返回签名会波及多处调用，不建议）；③ 成功路径同样应调 `mark_key_success`，否则聊天成功永远不会把 `fail_count` 清零；④ 可选增强：本轮失败后用「下一把可用 Key」**原地重试一次**再报错——但 `Agent` 的 llm 实例是构造期定死的，需要重建 Agent，属独立改动。**踩坑留档**：① 空闲内存 1.84GB 时单文件 e2e 仍稳（8.7s），但全量套件在该内存下会大面积 30s 超时，跑全量前先看 `FreePhysicalMemory`；② `Switch` 的 `aria-label` 里带掩码（`启停 sk-e****1111`），用它定位比 index 稳；③ 断言「面板内不含明文 Key」时要注意 textarea 在 `setBatch("")` 后确实已清空，否则这条会假绿。 | 用户 2026-09-20 提出（P8 交接收尾） | ✅ 验收完成；缺口见待办 15 |
...
| 2026-09-21 | **✅ 供应商能力维度（P5）落地：四个占位 Tab 转真功能**（用户截图点名「这些功能」）。**后端**（`5cba4cf`）：`ProviderConfig.capability ∈ {chat,stt,tts,embedding,rerank}`（默认 `chat`，`server_default="chat"` 兼容存量）→ 迁移 `f4a7c2d8e1b3`：加列 → 按 `(user_id, capability)` 用 `DISTINCT ON … ORDER BY updated_at DESC` 去重 → 换**部分唯一索引** `uq_provider_configs_default_per_user_capability`（降级反向）——**「设为默认」的互斥范围从 `user_id` 收窄到 `(user_id, capability)`：每种能力各自可有一条默认源**；`ProviderManager` 的 `get_active_config` / `chat_with_fallback` / `list_providers` / `clear_other_defaults` / `create_provider` 全部带 `capability`，`delete_provider` 的补位**只在本能力内**挑接替者（且 `capability` 必须在 `flush()` 前取出——flush 后对象已失效）。**API**：`Capability = Literal[...]`（与 `models.py` 的 `PROVIDER_CAPABILITIES` 双份定义、注释互指）、`ProviderCreate.capability`、`GET /providers?capability=embedding`；**PATCH 不允许改能力**（改能力等于换语义，会留下跨能力的默认源）。**测试**：新增 `tests/provider/test_capability.py` 9 条（默认 chat / 每能力各自默认 / 清默认只清同能力 / `get_active_config` 过滤 / 列表过滤 / 删除只在同能力补位 / 无同能力接替者时保持无默认 / **同能力第二条默认撞 IntegrityError** / 不同能力默认共存）+ `tests/api/routes/test_providers.py` 6 条（含 422 非法能力、嵌入默认不抢对话默认）。**前端**（`22872a2`）：新建 `components/Providers/capabilities.ts`（`CAPABILITY_TABS` 五页 / 文案 / 模型名占位符）、`useProviders(capability)`（queryKey `["providers", capability ?? "all"]`，失效仍走前缀匹配）、Tab 改真按钮（`data-testid="provider-tab-*"` + `aria-pressed`）、切栏清空右侧选中、新增弹窗与详情按能力裁剪（**非 chat 不渲染「视觉能力」行、也不提交 `supports_vision`**；标题 / 默认勾选 / 模型名占位符带能力名）、空态文案带能力名。**实机验收**（`6cbe370`）：新增 `frontend/tests/provider-capability.spec.ts` 2 条在 8000 实机 dist 通过——五个 Tab 全部可见且可用、在「嵌入」栏新建并设默认后切回「对话」栏**不串台且原有默认未被抢走**、详情文案为「嵌入使用的默认模型」且无「视觉能力」、模型名确实落库（`toHaveValue`）、自清理 + 幂等；`onboarding.spec.ts` 的弹窗标题断言跟进为「新增对话模型源」。**全量 e2e 75 passed**（`--workers=1 --timeout=120000`，空闲内存 1.8GB），后端全量非集成 **752 passed / 18 deselected**。**踩坑留档**：① `tests/conftest.py` 用 `create_all` 建表，**不会 ALTER 已存在的表** → 模型加列后必须**手工给宿主 `test_app` 打 DDL**，否则 76 条用例齐报 `UndefinedColumn`；② **localhost:5432 是宿主自带的 PostgreSQL 18.3，不是 compose 里的 `db` 容器**（override 故意不映射 5432，因为宿主已占用）——连错库会把宿主 `app` 库改坏，脚本里须显式 `make_url(...).set(database="test_app")`；③ `str(settings.SQLALCHEMY_DATABASE_URI)` 会把密码渲染成 `***` → 必须 `.render_as_string(hide_password=False)`；④ 迁移在容器 `prestart` 实跑通过（`e2f6b3c7a9d4 -> f4a7c2d8e1b3`），存量 2 条源回填 `chat`、`?capability=embedding` 返回 `[]`；⑤ **e2e 切能力栏后不能立刻 `count()`**——切栏会按能力重新拉列表，`count()` 不等待会把「尚未加载」误判成「这一栏没有目标行」，清理逻辑要先 `expect(任一行.or(本栏空态)).toBeVisible()` 等数据到位（首轮就是这么假绿过一次）。**遗留**：P5 承诺的「RAG 读时兼容 + `.env` 可迁移横幅」随 P9a 一起做（见待办 16） |

| 2026-09-21 | **✅ P9a 嵌入 / 重排序接入「模型源」体系（待办 16 之①闭环）**（`ecde963`）。新增 `core/knowledge_base/endpoints.py` 作为**凭据解析层**：`resolve_embedding_function(session, user_id)` / `resolve_rerank_endpoint(session, user_id)` —— **有 `user_id` 且本人配了 `capability=embedding` / `rerank` 的默认源就走它，否则回落 `.env` 的 `EMBEDDING_*`**（存量部署零行为变化）。`ENABLE_RERANK` 只闸 `.env` 路径：**显式配了 rerank 源则无视开关直接用**（源本身就是用户意图）。**user_id 透传链**：`builtins/kb_query.py` 与 `/kb/{id}/query` 路由下传 `current_user.id` → `KBManager.query(user_id=…)` → `_get_retriever(user_id=…)` / `index_document`（`record.user_id`）→ `VectorStore(embedding_function=…)` + `HybridRetriever(rerank_endpoint=…)` → `rerank(query, docs, top_n, endpoint=…)`，**索引与检索走同一源**。顺带修 `ProviderManager.get_embedding_model` **未按 `capability="embedding"` 过滤**的隐患（原会拿到对话默认源）。启动横幅后 `log_embedding_migration_hint()` 仅当日志：`.env` 两个值都在时 `INFO` 提示可迁移。**测试**：新增 `tests/core/knowledge_base/test_endpoints.py` 22 条（顶层绕过外键 + 清 `ProviderKey/ProviderConfig` + autouse monkeypatch `.env` 凭据；含「对话默认源不当嵌入源」「开关关掉时仍优先 Provider」「全部 Key 冷却后回落 `.env`」）+ `test_reranker.py` 3 条（显式 endpoint 优先于 `.env` 与开关 / 出错仍降级 / `HybridRetriever` 透传）+ `tests/provider/test_capability.py` 2 条（`TestEmbeddingLookup`）+ `test_manager.py` 1 条（`query` 把 `user_id` 透传给两个解析器）、并给 `test_upload_sets_done` 补 `resolve_embedding_function` 打桩与断言（FakeSession 查不了库）——**后端全量非集成 776 passed / 18 deselected**。**实机验收（8000 容器 A/B）**：**(A)** 未配嵌入源 → `POST /api/v1/kb/{小北}/query` = **200**，返回真实 Milvus 命中（`.env` 路径完好）；**(B)** 新建黑洞嵌入源（`base_url=http://127.0.0.1:9/v1`、`is_default=true`）→ `docker compose restart backend` 清 `_retriever_cache` → 同查询 **500**、日志出现 httpx 连接失败 traceback——**证明链路真的改走配置源**；**(C)** 删除该测试源 → 再查回 **200**，确认回落生效、用户配置已复原。**踩坑留档**：① **容器跑的是「烤进镜像」的后端代码，不是 bind mount**——`docker logs` 里那句 `Will watch for changes in directories: ['/app/backend']` 监听的是非挂载目录，`reload` 永不触发；后端改动**必须** `docker compose up -d --build backend`（`prestart` 会跑 alembic）。② **应用日志不进 `docker logs`**：`setup_logging` 只挂 `RingBufferHandler`、没有 console handler，应用层日志只能从 `GET /api/v1/logs?limit=N`（`/console` 页，超管）看；`docker logs` 只有 uvicorn 自身 stdout。③ `_retriever_cache` 的 **key 保持 `kb_id` 单键不变**（`invalidate_kb_cache(kb_id)` 依赖它），解析出的 endpoint 挂在 retriever 对象里；**换嵌入源需重建索引**（已文档化，不在缓存层防御）。④ Python 的 `/tmp` 在 win32 上是 `C:\tmp`，与 Git Bash 的 `/tmp` **不是一个目录**——helper 文件用 bash 写、API 响应用 `python -c` 读 stdin。 |
| 2026-09-21 | **✅ 项目名拼写更正 + 远端仓库收口**（纯文档/运维，无代码改动）。用户指出项目名是 **IrsBot**，仓库与文档里出现的 `IstBot` / `istbot-` 均为笔误（代码侧 README、Logo、compose、`app/main.py`、`docs/OPERATIONS.md` 一直是对的）。**① 文档**：`plan/istbot-implement-plan.md` → `plan/irsbot-implement-plan.md`（`git mv`），全仓 10 个文件 30 处引用同步改指新名，改后 `git grep -i istbot` 零残留；`irsbot-implement-plan.md` 第九节那行「文件重命名」历史说明补了更正轨迹（`progress.md` → `istbot-…` → `irsbot-…`），不抹掉当时确实叫错过。**② 远端**：`origin` 原为 full-stack-fastapi-template 的 fork（默认分支 `master`，旧 `feat/agent-platform` 1347 提交，与本地 130 提交线**无共同祖先**）。force push 被安全策略拦下，改走零破坏路径：本地线推到新分支 `main` → 用户到 Settings → 概述 把默认分支切成 `main` → 删除 `master` / 旧 `feat/agent-platform` / 4 条 dependabot 分支；删除前先 `git fetch origin '+refs/heads/*:refs/backup/istbot-old/*'`，旧历史完整留在本地 `refs/backup/istbot-old/*`。**现在远端只有 `main` = `32c18d0`**，本地 `feat/agent-platform` 跟踪 `origin/main`。**待办**：仓库名本身要用户在 https://github.com/mzhnx-go/IstBot/settings 改名为 `IrsBot`（GitHub 会自动重定向旧 URL）；确认不再需要旧历史后清 `refs/backup/istbot-old/*` |
| 2026-09-21 | **✅ P9b / P9c 语音能力落地（待办 16 之②③闭环，P9 系列全部完成）**。**后端**（`8ab93d8`，`fix` 见 `a195059`）：新增 `core/agent/audio.py` —— `resolve_audio_endpoint(session, user_id, capability)` 取当前用户 `capability="stt"/"tts"` 的**默认源**，**无 `.env` 回落**（这两类能力此前从未实现、没有存量配置要兼容；没配就 400 给出可执行指引，非 openai 类型明确拒绝——anthropic/gemini 没有音频端点，不装作能用）；`transcribe()` 走 multipart `POST {base_url}/audio/transcriptions`、`synthesize()` 走 JSON `POST {base_url}/audio/speech`，上游失败统一包成中文 `AudioUpstreamError`。新增 `routes/audio.py`（薄转发 + `AUDIO_MAX_BYTES=25MB` 边读边计数 / `AUDIO_TTS_MAX_CHARS=2000`，先校验后取凭据）。**前端**（`b9aba84`）：`useAudioRecorder`（MediaRecorder 封装，**`stop()` 用 Promise 交出完整 Blob**——`recorder.stop()` 只是请求停止、数据要等 `onstop`，调用方必须拿到完整 Blob 才能发请求；任何退出路径都停 track：卸载/取消/出错，否则标签页会一直亮录音指示）、`useVoice.ts`（`useTranscribe` 录音→上传→返回文本，**只填进输入框不自动发送**；`useSpeak` 先 `toSpeechText()` 把 Markdown 洗成纯文本——代码块整体略过、剥掉 `#`/`*`/反引号，否则会把标记念出来——再整段合成、blob URL 播放）。输入框新增麦克风按钮（录制中变红+脉冲 + 「正在录音…」提示 + placeholder 切换），回复操作条新增「朗读」。**客户端按新 openapi 重新生成**（`frontend/openapi.json` 与 `frontend/dist` 均在 .gitignore 里，不入库）。**测试**：后端新增 37 条（`tests/core/agent/test_audio.py` 20 + `tests/api/routes/test_agent_audio.py` 16 + detail 兼容 1），全量非集成 **812 passed / 18 deselected**；e2e 新增 `frontend/tests/chat-voice.spec.ts` 2 条，**全套 77 passed**（`--workers=1 --timeout=120000`）。**实机验收（8000 容器）**：① 未配源时两个端点分别回 `400 尚未配置语音转文字/文字转语音模型源…`；② 建一个黑洞源（`base_url=http://127.0.0.1:8000/api/v1`，即指向后端自己的非音频路径 → 上游 404）后，STT 回 **502 语音识别失败**、删源后回 400——**证明后端确实拿着配置源在发请求**（没用配置源的话会是 400 而不是上游失败）；e2e 里固化了同一手法（自建源 → 断言 → 自删）。**踩坑留档**：① **`page.request.fetch` 传字符串 body 不带 `Content-Type`**，FastAPI 会按字符串校验请求体 → 422 `model_attributes_type`；传对象（`data: {...}`）让 Playwright 自己序列化并带 JSON 头才对。② 无头 Chromium **没有麦克风也没有选设备弹窗**，录音链路必须 `addInitScript` 打桩 `MediaRecorder` + `getUserMedia`（产出的假音频 Blob 仍会真的走 multipart 打到 8000）。③ 兼容网关的错误体形状不止一种：把 base_url 指向 FastAPI 系网关时回的是 `{"detail": ...}` 而非 `{"error":{"message":...}}`，只认后者会把整段 JSON 当文案展示给用户（实机 A/B 撞到，已修 `a195059` —— 这也是**实机验收的价值**：单测里 MockTransport 是我自己写的，喂的全是 OpenAI 形状）。④ `navigator.mediaDevices` 只在安全上下文存在，127.0.0.1 算安全上下文、局域网 IP 不算——按钮保留可见但在不支持时禁用并给 title 说明。**未覆盖（如实记录）**：本机没有可用的 STT/TTS 上游凭据，因此**真实语音的转写质量、真实音频的播放效果无法自动化**；「朗读」按钮的 UI 也未能自动化断言（它只出现在助手消息上，而全套 e2e 刻意不依赖真实模型回复）——已人工核对代码路径，待用户配上真源后补一次人工听感验收。 |
