...
**在哪改**：登录后进入 **`/admin` 页**（仅管理员可见）→ 顶部「**部署模式**」卡片 → 选择「单用户 / 多租户」。

**保存即生效，无需重启。**

| 模式 | 行为 |
|---|---|
| **单用户**（默认） | 登录页不显示「注册」入口；匿名 `POST /api/v1/users/signup` 一律 403。账号只由管理员在 `/admin` 页创建 |
| **多租户** | 任何能访问此服务的人都可以在登录页自助注册账号 |

**不会受影响的东西**（两种模式完全一致）：管理员建号、`/admin` 页可见性、**每个用户的数据归属隔离**
（Provider / 知识库 / 会话 / MCP / 人设都按 `user_id` 隔离，A 用户永远看不到 B 用户的模型源与密钥）。

> 切换模式**不会删除或停用任何已有账号**。从多租户切回单用户后，已存在的账号仍可正常登录。

### ⚠️ 与 `.env` 的优先级

```
app_settings 表（网页开关）  >  .env 的 USERS_OPEN_REGISTRATION  >  代码默认值（false）
```

也就是说：**只要在网页上切过一次，之后改 `.env` 就不再生效**（会被表的记录覆盖）。

- 网页上切回「单用户」即可随时关闭
- `.env` 的值只在**从未在网页上改过**时作为初值使用。若发现「改了 `.env` 没反应」，就是这条优先级在起作用
- 网页上的开关卡片会在两者不一致时显示告警

### 直接看当前状态（不走网页）

```bash
# 当前生效值
curl.exe -s http://localhost:8000/api/v1/utils/public-settings

# 直接改（需要管理员 token；一般用网页即可）
# docker compose exec -T db psql -U postgres -d app -c "select * from app_settings;"
```

### 首次开放自助注册前的检查

开启「多租户」等于**对任何能访问该端口的人开放注册**，注册进来的账号会消耗**你配置的模型源（你的 API Key）**。

- 本地 `localhost` 使用：风险可控
- 若通过内网 / 公网暴露过端口：**先确认访问控制**（防火墙 / 反向代理鉴权），再开启
